...
module.exports.execute = async(client, message, args, ayar, emoji) => {
  	let raw = db.get("raw");
	  let embed = new MessageEmbed().setAuthor(message.author.username, message.author.avatarURL({dynamic:true})).setColor("RANDOM").setFooter("SAVARONA ❤️ PAİ").setDescription(`${message.guild.name} sunucusunda son __14 günlük__ kullanıcı aktiflik verileri. \n Bu verilerin kayıt olma şekli Türkiye saati ile yapılmaktadır.`);
